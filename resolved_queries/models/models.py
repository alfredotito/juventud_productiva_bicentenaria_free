...
class resolvedQueries(models.Model):
    ...
    def run_resolved_queries(self):
        irModel=self.env['ir.model']
        logfile=tools.config.get('logfile')
        msg='You must configure the absolute path of the '
        msg=msg+'logfile in the configuration file.'
        if not logfile:
            raise UserError(_(msg))
        if not path.isfile(logfile):
            raise UserError(_(msg))
        if not os.access(logfile, os.R_OK):
            msg='The %s file does not have read permission.' % logfile
            raise UserError(_(msg))
    ...
    def run_resolved_queries(self):
        ...
        for record in self.resolved_queries_ids:
            if record.search_parameter:
                ...
                record.table=model._table
                record.total_row=model.search_count([])
                cmd = 'grep -rn %s %s | wc -l' % (record.search_parameter,logfile)
                process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
                result =process.communicate()
                total=result[0][:-1].decode()
                record.total=total
                selfTotal+=int(total)
                cmd = 'grep -rn %s %s' % (record.search_parameter,logfile)
                process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
                result = process.communicate()[0]
                record.query_file = base64.b64encode(result)
                to_date=fields.Date.today()
                record.store_fname="%s_%s_t_%s.txt" %(record.model_id.name,to_date,str(total))
